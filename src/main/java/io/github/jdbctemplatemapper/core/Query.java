...
public class Query<T> implements IQueryFluent<T> {
    ...
    /**
     * The join table (associative table) for the hasMany through (many to many)
     * relationship
     * 
    ...
    /**
     * The join columns for the owning side and the related side for hasMany through
     * relationship
     * 
    ...
    public List<T> execute(JdbcTemplateMapper jdbcTemplateMapper) {
        ...
        QueryValidator.validate(jdbcTemplateMapper, ownerType, relationshipType, relatedType, joinColumnOwningSide,
                joinColumnManySide, propertyName, throughJoinTable, throughOwnerTypeJoinColumn,
